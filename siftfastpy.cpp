...
inline object toPyArrayN(const float* pvalues, int N) {
    ...
    if( pvalues != NULL ) {
        memcpy(PyArray_DATA(pyvalues),
            pvalues, N*sizeof(float));
    }
...
class SiftFastImage {
    public:
        SiftFastImage(int width, int height) : width(width), height(height) {
            BOOST_ASSERT(width>0&&height>0);
            stride = (width+3)&~3;
            vimage.resize(height*stride);
        }

        SiftFastImage(object oimage) {
        ...
        virtual void SetData(object arr) {
...
class PyImage : public SiftFastImage {
    public:
        PyImage(int width, int height) : SiftFastImage(width, height) {}
        PyImage(object oimage) : SiftFastImage(oimage) {}
        const PyImage() {}

        object Keypoints(SiftFastImage* im = NULL) {
            if (im == NULL) { im = (SiftFastImage*)this; }
            ...
            return make_tuple(
                static_cast<numeric::array>(handle<>(pyframes)),
                static_cast<numeric::array>(handle<>(pydesc)));
        }
};

object PyGetKeypoints(SiftFastImage& im) {
    return PyImage().Keypoints(&im);
}
object PyGetKeypoints(numeric::array oarray) {
    SiftFastImage pimage(oarray);
...
struct int_from_int {
    ...
    static void* convertible(PyObject* obj) {
        ...
        if (!PyString_Check(obj) && newobj) {
            ...
            if (newobj) { Py_DECREF(newobj); }
            PyErr_Clear();
            return 0;
        }
    }

    static void construct(PyObject* _obj,
...
BOOST_PYTHON_MODULE(siftfastpy) {
    ...
    object (*pkeypoints1)(SiftFastImage&) = PyGetKeypoints;
    ...
    class_<SiftFastImage>("SFImage", no_init)
        .def(init<int, int>())
        .def(init<object>())

        .def_readonly("width",  &SiftFastImage::width)
        .def_readonly("height", &SiftFastImage::height)
        .def("SetData",         &SiftFastImage::SetData,        args("array"))
        .def("setData",         &SiftFastImage::SetData,        args("array"))

        .def_pickle(Image_pickle_suite());


    class_<PyImage, bases<SiftFastImage> >("Image", no_init)
        .def(init<int, int>())
        .def(init<object>())
